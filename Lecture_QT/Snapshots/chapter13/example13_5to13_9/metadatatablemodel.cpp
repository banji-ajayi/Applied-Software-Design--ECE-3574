...
MetaDataTableModel::MetaDataTableModel(QList<QSharedPointer<MetaData> > list, QObject *parent)
...
bool MetaDataTableModel::dropMimeData(const QMimeData *data, Qt::DropAction action, int row, int col, const QModelIndex &parent) {
    Q_UNUSED(col);
    Q_UNUSED(parent);
    ...
    if ((row < 0) || row >= m_list.size()) {
        ...
        beginInsertRows(parent, row, row+lines.size() -1);
