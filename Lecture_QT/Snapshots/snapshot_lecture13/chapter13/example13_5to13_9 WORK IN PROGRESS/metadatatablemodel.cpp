...
MetaDataTableModel::MetaDataTableModel(QList<QSharedDataPointer<MetaData> > list, QObject *parent)
...
QVariant MetaDataTableModel::headerData(int section, Qt::Orientation orientation, int role) const {
...
bool MetaDataTableModel::setData(const QModelIndex &index, const QVariant &value, int role) {
    if (!index.isValid())
        return false;
    qDebug() << QString("setData(%1,%2, %3)").arg(index.row()).arg(m_columnProps.at(index.column())).arg(value.toString());

    if(role == Qt::UserRole) {
        quint64 decodePointer = index.data(Qt::UserRole).value<quint64>();
        MetaData* md = reinterpret_cast<MetaData*>(decodePointer);
...
bool MetaDataTableModel::dropMimeData(const QMimeData *data, Qt::DropAction action, int row, int column, const QModelIndex &parent) {
    Q_UNUSED(col);
    Q_UNUSED(parent);
    ...
    if ((row < 0) || row >= m_list.size()) {
        ...
        beginInsertRows(parent, row, ro w+lines.size() -1);
